...
This section goes into advanced configuration for passing through PCI devices in a way that allows all GPUs to be used for VMs as well as returns the Virtual Terminal video output when VMs are not in use. 

*If you are only using 1 passthrough VM* ***or are dedicating a separate GPU for console output*** ... you can probably skip this section. Feel free to test your VM out (see [06 - VM Windows 10](06.VMWindows10.md) for creating one) before coming back here if you want to add this hookscript. 

### 05.D.i. driverctl Install and Use

You won't need to manually use `driverctl` once the hookscript is running, but this section will give you information on how it works. 


* **NOTES:**
    + 'driverctl' is a part of the Debian Bullseye repositories. I use it *instead of blacklisting*. It allows reassigning devices repeatedly without rebooting while retaining use of a GPU for virtual terminal when not on a VM (vs blacklisting and disabling those modules ... ([originally found here](https://www.heiko-sieger.info/blacklisting-graphics-driver/)).
    + *If you override the GPU used for the virtual terminal,* ***video output for the virtual terminal will stop***, but will be usable again when the VM is stopped. 
    + My system has 2 different GPU models, which makes it easy to know which is on each PCI id. **If you have 2 identical models** you may need to consult another guide to fix issues (but the following may just work, I can't test)
...
* *No easy copy-paste block for these commands to remind you to update for* ***your IOMMU Ids*** before using.* The hookscript will be much more automated, this is just to get you used to the package being used by it. 
