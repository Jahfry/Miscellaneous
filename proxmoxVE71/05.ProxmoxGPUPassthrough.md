...
* Add kernel boot parameters (if you have other options on your cmdline, add the following, don't replace the other items)
	...
	- *If you want to see detailed boot info*, leave `quiet` out of the command, but using it speeds up boot
...
#
# Enable hookscript on a VM via:
# `qm set <vmid> -hookscript local:snippets/driverctl.pl`
#
# Disable hookscript via:
# `qm set <vmid> --delete hookscript`
#
# Behavior:
# * On VM start, check to see if passthrough PCI devices are currently bound to vfio-pci
#   + if no, bind to VFIO-PCI and start VM
#   + if yes, assume another VM is active on the devices, don't allow VM to start
# * On VM stop, unbind passthrough PCI devices
#
# Details:
...
print "\n\pre-start\n";
for (@pci_devices) {
  my $driverctl_cmd = "${driverctl} --nosave set-override $_ vfio-pci";
  print "Trying to override " . $_ . " to vfio-pci\nCommand: `$driverctl_cmd`:\n";
  my $exit_code = system($driverctl_cmd);
  if ($exit_code != 0) {
    print "failed with exit code: $exit_code ... aborting\n\n";
    exit 1;
  } else {
    print "SUCCESS\n\n";
  }
...
# root@gyges:/var/lib/vz/snippets# lspci -nnks 0b:00
# 0b:00.0 VGA compatible controller [0300]: NVIDIA Corporation Device [10de:2208] (rev a1)
#         Subsystem: NVIDIA Corporation Device [10de:1535]
#         Kernel driver in use: nouveau
#         Kernel modules: nvidiafb, nouveau
# 0b:00.1 Audio device [0403]: NVIDIA Corporation GA102 High Definition Audio Controller [10de:1aef] (rev >
#         Subsystem: NVIDIA Corporation Device [10de:1535]
