...
#   * On VM stop, unset devices for a different VM/container to use
#   * Failure to read VM conf or set with `driverctl` will prevent VM from starting
#
# Usage:
...
if (! -e $driverctl) {
  print "\nERROR: $driverctl does not exist. Aborting.\n";
  exit 1;
} elsif (! -x $driverctl) {
  print "\nERROR: $driverctl is not executable. Aborting\n";
  exit 1;
}
...
if (-r $config_file) {
  ...
  print "\nERROR: failed to read $config_file. Aborting.\n";
  exit 1;
}
