...
* Add kernel boot parameters (if you have other options on your cmdline, add the following, don't replace the other items)
    ...
    + The string to add: `quiet amd_iommu=on iommu=pt`
        ...
        - You can add `fbcon=map:1` if you have 2 GPUs and want virtual terminal output on the second GPU slot (it will still be usable for a VM, disabling the terminal output only while in use)
          *Example:*  
...
* ***Open [hookscript-driverctl.pl](hookscript-driverctl.pl)***  *in a new browser window*
	 ...
	 + Paste in the script
     + Verify you only pasted the script lines, not other parts of the page
