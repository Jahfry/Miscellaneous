...
* Add kernel boot parameters (if you have other options on your cmdline, add the following, don't replace the other items)
    + Doing this is **critical** to following steps
    + systemd
        - Add this to '/etc/kernel/cmdline': " quiet amd_iommu=on iommu=pt video=efifb:off"  
        Example:  
    ...
    + ' video=efifb:off' above stops the system from using framebuffer on the console
