...
* Add kernel boot parameters (if you have other options on your cmdline, add the following, don't replace the other items)
    ...
    + The string to add: `quiet amd_iommu=on iommu=pt`
        - The above is **critical** to following steps working
        - You can also add `fbcon=map:1` to the above if you have 2 GPUs and want VT monitor output on the second GPU (allows VT to be active while the big GPU is used by a VM, is temporarily disabled when using the small GPU with a VM).  
          Example:  
	  > `quiet amd_iommu=on iommu=pt fbcon=map:1`
        ...
        - Example including ALL of the above:  
            ...
            (you can add all to see if your system boots and attaches GPUs properly, then test removing some)
    + systemd
    ...
    + grub 
        ...
        - This is important when using the same GPU for a VM that the system displays console video on. 
        - If you're **dedicating a GPU for console video** use you can leave that out to have better console video, but doing this while using 'driverctl' with the GPU through as I do below can result in failure to boot if you don't make sure 'driverctl' reattaches 'nouveau' to the console before **any** boot. 
