...
*This section* ***VERY Optional*** *and modifies system files in a way that can be overridden in Proxmox updates.* 

**NOTES:**

* These just tune the UI
* They **will** need to re-apply after some Proxmox updates (I had to do just that after my first system update, which is why we do that first)
* You can revert these prior to updating but doesn't seem needed
    ...
    + Dannyda's script
        - Keep a copy of the original file:  
            ...
            * To revert to the original:  
                `mv /usr/share/javascript/proxmox-widget-toolkit/proxmoxlib.js.orig /usr/share/javascript/proxmox-widget-toolkit/proxmoxlib.js && systemctl restart pveproxy.service`
        - [Install from the page](https://dannyda.com/2020/05/17/how-to-remove-you-do-not-have-a-valid-subscription-for-this-server-from-proxmox-virtual-environment-6-1-2-proxmox-ve-6-1-2-pve-6-1-2/) ... *Multiple versions listed, use the one for your Proxmox version ("6.4-6 through 7.1-12 and up" as of this writing), paste "Easy method" into a shell. The command is long so make sure you copy it all. 
...
*I recommend doing the following if running a single Proxmox system on consumer hardware.* If using multiple Proxmox systems and/or Enterprise class storage, you probably don't want some of these. I've put details as to what each step does for you to decide. 
...
* **To get an idea of how much is being written every few seconds before these changes:**
    + `zpool iostat 2`
        - this adds more to the writes when running, so the actual total below is lower
        - changing the interval affects how much writing is happening (change "2" to "1" to see write total double, change to "5" to see write total less than half of "2", etc)
    + This runs the command every 2 seconds
    + On my system at this fresh point it is around 400-500K every 4-5 seconds
    + Check again after making the following changes
...
Decide which is more important to you. You can read more about it [here](https://linuxfun.org/en/2021/01/01/what-log2ram-does-en/) along with [this](https://forum.proxmox.com/threads/proxmox-usb-bootstick-mit-log2ram-oder-folder2ram.76583/) (in German but Google Translate does a good job). The second link mentions possibly switching to 'folder2ram' instead of 'log2ram'. More on that later but I stuck with 'log2ram'. 


**NOTES:** 

* The README for log2ram mentions a config option for 'RSYNC', this is an older version option. Current version of log2ram will use rsync as default if it is available, which it is by default on Proxmox VE. I've filed an [issue for this](https://github.com/azlux/log2ram/issues/188) to update the README.
* The config talks about ZRAM / ZL2R ... this appears to be often used on raspberry pi systems, I haven't used it. 

#### 03.C.ii.a. initial install

**NOTE:** This will add a new apt repository, 'http://packages.azlux.fr/debian/', be aware of this if you have security concerns with 3rd party packages. 
...
* *If you only want logs in RAM*, ***not anything that is actively used by the PVE UI or HA services***, reboot now
...
* edit **'/etc/log2ram.conf', parameters I changed:**
    + Read comments in the file for more information
    + 'SIZE=40M' to 'SIZE=400M'
    ...
    + 'PATH_DISK="/var/log"' to 'PATH_DISK="/var/log;/var/lib/pve-cluster;/var/lib/rrdcached"'
    + 'LOG_DISK_SIZE=100M' to 'LOG_DISK_SIZE=1000M'
        - Done to match the increase in RAM use. 
        - I'm not 100% sure this needed to be done, have filed a [question against it](https://github.com/azlux/log2ram/issues/188), will update depending on what I hear back. 
...
* I seem to remember seeing a comment that Proxmox can override this file with a default version. Need to keep an eye on it. Doesn't seem like this is a problem as the comments in 'sysctl.conf' reads like editing is fine
...
* **Reboot** after doing whatever you want from above ... *can wait until you've tuned ZFS RAM below and come back to save a reboot*
* Verify the swappiness, if changed, via `sysctl vm.swappiness`
* With all changes above, `zpool iostat 2` (after reboot) shows 400-500K every minute instead of every 4-5sec
   + This can probably be lowered more but I'm OK for now
   + Rough estimates on this SSD with 150TBW in the warranty (which ends after 5 years) would be 150+ years of use if nothing else hits the drive
...
Fixed. After rebooting (*no need to do reboot now*), `dmesg | grep regulatory` shows this:
...
***After rebooting an infinite loop on the console with Bluetooth disconnect/failed/reset errors and fails to boot.***
