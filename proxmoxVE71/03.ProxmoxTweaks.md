...
At this point `zpool iostat 2` shows <>500K being written once per minute to 'rpool' (the boot SSD). This is down from 1MB every few seconds. `iotop -o -a` isn't informative. 

Can we get it down further by investigating pveproxy?

(move this to the right section later, just scribbling)
