...
* **Optional*** and modify system files in a way that can be overridden in Proxmox updates. 
...
**NOTE:** I've stopped using these as the pop-up likes to come back and some versions can break the UI. But if you want it, I've listed the one that worked better for me. The pop-up isn't that intrusive. 
...
* ~~Follow the instructions on [John's Computer Services](https://johnscs.com/remove-proxmox51-subscription-notice/).~~
    + don't use for now as it blocks some UI elements. I've reported the problem.
* Alternately if you want to keep this change active through updates, [PVE Nag Buster](https://github.com/foundObjects/pve-nag-buster/) might appeal.
    + found in this [Reddit thread](https://www.reddit.com/r/Proxmox/comments/tgojp1/removing_proxmox_subscription_notice/)
    + *I chose not to use this to keep from adding another repository that will auto-update a non-Proxmox/Debian package.* Your choice may vary. 
...
* I think I remember seeing where Proxmox can override this file with a default version. Need to keep an eye on it. 
* [4 year old issue](https://forum.proxmox.com/threads/swappiness-question.42295/) with Proxmox that I need to verify is fixed (once I have containers running)
