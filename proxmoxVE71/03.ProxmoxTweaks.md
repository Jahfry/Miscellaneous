...
* > 03 - Proxmox Tweaks (***this page***)
    ...
    + [03.E. Importing Existing ZFS Pool](#03e-importing-existing-zfs-pool)
    + [03.F. Fix Missing Drivers](#03f-fix-missing-drivers)
        - [03.E.i. 'regulatory.db'](#03fi-regulatorydb)
        - [03.E.ii. FAILED fixes for 'iwlwifi' and 'thermal_zone2'](#03fii-failed-fixes-for-iwlwifi-and-thermal_zone2)
    + [03.G. Useful Utilities](#03g-useful-utilities)
---
...
* 'tmpfs /var/lib/rrdcached' ... Proxmox uses this for the graphs in the UI (and possibly other places), frequently written
    + **NOTE:** after moving rrdcached to RAM I've noticed there are gaps in the graphs (like on "Summary") in the Proxmox UI when rebooting.
        - This gap can be quite long if doing multiple reboots, 15+ minutes of missing graphs
        - I assume something with syncing the data to disk doesn't work as expected with log2ram
        - Not critical for me as the graphs update fine once the system is running for a minute or two but if this bugs you, don't move rrdcached to RAM. 

The folders I decided NOT to move to RAM for now that might save more writes:
...
[Some good reading](https://www.cyberciti.biz/faq/how-to-set-up-zfs-arc-size-on-ubuntu-debian-linux/)

* To see what your ZFS ARC is *currently* able to use
...
On a freshly installed server without anything extra running you might not see the RAM usage. If you want to see it in action, set up a long file transfer (for me it was copying 8TB from an old drive to my 'xfer' pool, showing 64GB/50% RAM use for hours). As mentioned, it's not a big deal, just interesting to see. 

See [this](https://www.solaris-cookbook.eu/solaris/solaris-10-zfs-evil-tuning-guide/) for Solaris ZFS background info on tuning ZFS. Don't use any commands there, but it's still good information to read quickly go through before tuning. 
...
* Some general ideas ([many from here](https://forums.servethehome.com/index.php?threads/how-much-ram-is-required-for-zfs.13463/):
    ...
    + However, I feel like 37GB is high and these are probably ok to mix together a bit. Final amount I'm setting: 32GB
        - That's up to 1/4 of my RAM (from 1/2)
        - *Reminder:* ZFS will free RAM from ARC as needed
        - My main gaming/authoring VM will get 32GB (yes, my main game actually needs it, Star Citizen loves RAM, and VirtIO memory ballooning on Windows is messy so just consider this permanently used ... I may go higher for authoring in a clone)
    ...
    + **IMPORTANT**: this does NOT take into account if you are going to use [ZFS deduplication](https://www.oracle.com/technical-resources/articles/it-infrastructure/admin-o11-113-size-zfs-dedup.html).
        - Add 2-3GB of RAM used for each 1TB of deduplicated storage ... I'm not going to jump farther into figuring out the exact right amount. 
...
* **WARNING:** The first block assumes you logged in as root are using the default 'bash' shell. 
    + If you aren't root yet, `su -`. 
    + Once done if you've changed root's shell, switch to bash via `bash` before pasting this.
* *Paste this and run it*
    + If you have more or less RAM, feel free to tweak the byte values
    + You might leave out the "zfs_arc_min=" line to allow ZFS to decide the minimum, rather than risking setting too low of a minimum
    + In my case the minimum I set here was already the default 4GB
...
* **Update initramfs with the change* (only choose **one** of these options). 
    + If you want to update ONLY the kernel currently running: `update-initramfs -u`
    + To update ALL kernels you have installed: `update-initramfs -u -k all`
...
## 03.E. Importing Existing ZFS Pool
...
### 03.G. Useful Utilities

* `iotop` ... see what is actively using system IO
...
* `iommu_list` ... which devices are in what IOMMU groups
    ...
    + grabbed from [ArchiWiki](https://wiki.archlinux.org/title/PCI_passthrough_via_OVMF) (no idea where it originated, I've seen it in a bunch of places)
    + be logged in as root and in a 'bash' shell first to create it, but after that non-root can use it
...
chmod 755 /usr/local/bin/iommu_list
```
