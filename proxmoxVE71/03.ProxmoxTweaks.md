...
**WARNING:** This assumes you are using the default 'bash' shell. If you've changed shells to something like sh or zsh, switch to bash (just enter `bash` on the command line) before pasting this in:
...
if test -f "$FILE"; then
    echo "$FILE exists."
#    cat << EOF > /usr/local/sbin/iommu-list
    cat << EOF > ./iommu-list
#!/bin/bash
shopt -s nullglob
