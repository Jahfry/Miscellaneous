...
*This section is* ***VERY Optional*** *and modifies system files in a way that can be overridden in Proxmox updates.* 

**NOTES:**

* These just tune the UI to be friendlier
* They **will** need to re-apply after *some* Proxmox updates (I had to do after first 7.1 system update, which is why I do that first)
* You can revert these easily

...

* *Dark Mode*
    + There isn't an official Dark Mode on your Proxmox UI
    + [PVEDiscordDark](https://github.com/Weilbyte/PVEDiscordDark) works well. Use the instructions there, including a shift+reload in your Proxmox UI aftwewards, and come back here. 
...
* *Remove "No Subscription" popups*
    + Dannyda's script
        - First make a copy of the original file:  
            ...
            * Dannyda's script creates a backup file 'proxmoxlib.js.backup', but if you apply the change twice for whatever reason before Proxmox has replaced it with the original version, the backup will just be the modified file
            * This adds another backup that will always be the original. 
        - [Install from the page](https://dannyda.com/2020/05/17/how-to-remove-you-do-not-have-a-valid-subscription-for-this-server-from-proxmox-virtual-environment-6-1-2-proxmox-ve-6-1-2-pve-6-1-2/)
	        - Multiple versions listed, use the one for your Proxmox version ("6.4-6 through 7.1-12 and up" as of this writing), paste "Easy method" into a shell
	        - *The command is long so make sure you copy it all* ***but only that command*** (it is very easy to accidentally select other parts of that web page)
        - Shift+Reload Proxmox UI to load
...
* **Get an idea of how much is written every few seconds without tuning:**
    + `zpool iostat 2`
        ...
        - this seems to add writes when running, so total below is a bit high, but will still show a massive difference after tuning
	...
	- On my system at this point I see 400-500K writes every 4-5 seconds
...
* **Reboot** after doing what you want from above (*to save reboot time you can wait until you've tuned ZFS RAM below and come back*).
...
* **Update initramfs* (only choose **one** of these options). 
    + Update ONLY the kernel currently running:  
      `update-initramfs -u`  
      *Or* ***update ALL installed kernels***:  
