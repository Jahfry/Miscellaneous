...
* > 03 - Proxmox Tweaks (***this page***)
    ...
    + [03.D. Minimizing SSD Wear](#03d-minimizing-ssd-wear)
...
#### Fix missing WIFI `regulatory.db`
...
#### Fix missing intel WIFI driver:

*Errors being fixed:*

> [    4.489255] iwlwifi 0000:05:00.0: firmware: failed to load iwlwifi-cc-a0-59.ucode (-2)

> [    4.489258] firmware_class: See https://wiki.debian.org/Firmware for information about missing firmware

> [    4.489260] iwlwifi 0000:05:00.0: Direct firmware load for iwlwifi-cc-a0-59.ucode failed with error -2

(repeats a number of times in `dmesg`)

```bash
apt -y install firmware-iwlwifi
```

### Fix missing realtek NIC firmware:

*Error being fixed:*

> [  142.889374] r8169 0000:07:00.0: firmware: failed to load rtl_nic/rtl8125a-3.fw (-2)

> [  142.889417] r8169 0000:07:00.0: Direct firmware load for rtl_nic/rtl8125a-3.fw failed with error -2

> [  142.889424] r8169 0000:07:00.0: Unable to load firmware rtl_nic/rtl8125a-3.fw (-2)

Note: This NIC will actually bind to the network with the default kernel driver, but since I'm not sure what might be missing and I don't mind non-free blobs much, I'm adding the firmware package. 

```bash
apt -y install firmware-realtek
```

* ***NOT fixing*** *this error, trivial as per [this discussion](https://patchwork.kernel.org/project/linux-wireless/patch/20200625165210.14904-1-wsa@kernel.org/):*

> [    4.385311] iwlwifi 0000:05:00.0: firmware: failed to load iwl-debug-yoyo.bin (-2)
